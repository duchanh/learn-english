...
import GtmNoScript from '@/components/layouts/GTM/GtmNoScript'
...
const CustomApp = ({ Component, pageProps, isMobile, categories }: TProps) => {
  ...
  return (
    <>
      ...
      <AuthContextProvider>
        <DeviceContextProvider isMobile={isMobile}>
          <SmartBannerContextProvider>
            <CategoryContextProvider categories={categories}>
      ...
      <Toast />
      <Script src={MEEEYID_SDK_URL} strategy='beforeInteractive' defer />
      <Script src={`${STATIC_DOMAIN}/js/auth.js`} strategy='beforeInteractive' defer />
    </>
  )
}
